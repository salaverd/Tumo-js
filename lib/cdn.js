...
function strangeSquare(side, step = 5) {
  ...
  if (typeof side !== "number" || side < 0) {
    console.error("Invalid side: must be a non-negative number.");
    return;
  }
...
function strangeCircle(radius) {
  ...
  if (typeof radius !== "number" || side < 0) {
    console.error("Invalid radius: must be a non-negative number.");
    return;
  }
  let x1 = currentX;
  let y1 = currentY;
...
//   ctx.save();
//   ctx.beginPath();
//   ctx.moveTo(x, y); // Start from the current position

//   while (l < length) {
//       let p = random(1, 5); // Random segment length
//       let h = random(1, 5); // Random stroke width
//       ctx.lineWidth = h;

//       let angleRad = (currentAngle * Math.PI) / 180;
//       x += p * Math.cos(angleRad);
//       y -= p * Math.sin(angleRad); // Negative Y because canvas Y grows downward
...
// Draws a strange zigzag or spiral line
function strangeLine1(length) {
  let l = 0;
  let x = currentX;
  let y = currentY;
...
// Draws a strange square pattern
function strangeSquare1(side, step = 5) {
  let x1 = currentX;
  let y1 = currentY;
  ctx.save();
  for (let x = x1; x < x1 + side; x += step) {
...
// Draws a swirling galaxy pattern
function strangeGalaxy1(radius) {
  let x1 = currentX;
  let y1 = currentY;
  ctx.save();
  for (let i = 0; i < 720; i++) {
...
// Draws a strange circle pattern
function strangeCircle1(radius) {
  let x1 = currentX;
  let y1 = currentY;
  ctx.save();
  for (let i = 0; i < 360; i++) {
